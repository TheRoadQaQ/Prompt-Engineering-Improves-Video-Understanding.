...
class VideochatgptDataset(Dataset):
    ...
    def __init__(self, questions, answers):
        self.questions = questions[:100]
        self.answers   = answers[:100]
...
def collate_fn(batch):
    ...
    prefixes = [x['prefix'] for x in batch if x is not None]
    return vid, v_id, qus, ans, prefixes
...
def run_inference(args):
    ...
    for i, (videos, video_names, questions, answers,prefixes) in enumerate(tqdm(dataloader)):
        for j in range(len(videos)):
            ...
            prompt = get_prompt(new_question)
    ...
    for k, o in enumerate(outputs):
        ...
        sample_set = {
            'new_question': metadata[k]['new_question'],
